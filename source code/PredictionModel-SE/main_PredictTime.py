...
from data_loader import build_ini_feature, load_data_from_file, test_data_split
######################################################
def parse_args_pool():
    ...
    parser.add_argument('--fig_save_dir', type=str, default='./Output_file/fig_v6/')
    parser.add_argument('--rst_save_dir', type=str, default='./Output_file/rst_v6/')
    # parser.add_argument('--hidden', type=int, default=8, help='Number of hidden units.')
    parser.add_argument('--BATCH_SIZE', type=int, default=200, help='Number of batch size in each iteration.')
    parser.add_argument('--MAX_ITER', type=int, default=280, help='Maximum iterations in a training set.')
    ...
    parser.add_argument('--PREDICT_N', type=int, default=500, help='Number of samples in calculating conditional density to predict Time.')
    parser.add_argument('--node_num', type=int, default=100, help='Number of nodes.')
    parser.add_argument('--embed_size', type=int, default=32, help='Number of nodes.')
...
#                 print(name, ':', parameters)
            ...
            for i in test_data_batch: 
                ...
                delta_t_sec = (delta_t.days*86400 + delta_t.seconds)/3600
                ...
                cur_time = date_timestamp_switch(i[2])/3600
                t_truth = date_timestamp_switch(i[4])/3600
                
                if(i[4] == datetime.datetime(2009, 6, 30, 23, 59, 59)):
                    useless = useless+1
                    continue
...
#                     f_pred = Lambda_pred * torch.exp(-Surviv_pred)
                    ...
                    f_pred = Lambda_pred * torch.exp(-(Lambda_pred)*(delta_t_stamp))
                    f_pred = f_pred.cpu()
...
#                 t_predict = cur_time + (t_next / PREDICT_N)
                ...
                with open(args.rst_save_dir + 'testrst.csv','a+') as f:
                    ...
                    data_row = [ith_slot+1,u,v, i[2] , i[4] , timestamp_date_switch(t_predict_np) ,v_condi,rankno,ranking[0].item()]
        ...
        print('Epoch: {:04d}'.format(epoch+1),
              'Time Slot: {:04d}'.format(ith_slot+1),
...
if __name__ == '__main__':
    ...
    feature = build_ini_feature()
    initial_data, train_data, test_data = load_data_from_file()
    ...
    G, Lambda_time, node_time = initialize_Graph_matrix( initial_data )

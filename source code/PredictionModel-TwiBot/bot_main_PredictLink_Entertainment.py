...
def parse_args_pool():
    ...
    parser.add_argument('--fig_save_dir', type=str, default='./Output_file/fig2/Entertainment/')
    parser.add_argument('--rst_save_dir', type=str, default='./Output_file/rst2/Entertainment/')
    # parser.add_argument('--hidden', type=int, default=8, help='Number of hidden units.')
    parser.add_argument('--BATCH_SIZE', type=int, default=300, help='Number of batch size in each iteration.')
    parser.add_argument('--MAX_ITER', type=int, default=190, help='Maximum iterations in a training set.')
    ...
    parser.add_argument('--node_num', type=int, default=3095, help='Number of nodes.')
    parser.add_argument('--embed_size', type=int, default=128, help='Number of nodes.')
    parser.add_argument('--cluster', type=str, default='Entertainment', help='cluster type')
...
def test(test_data_haveslot,model,Lambda_time,ite_test_arr, MAR_arr, HITS_10_arr, MAE_arr, args, node_time, test_timeslots):
    ...
    #先写入columns_name                             
    writer.writerow(["Slot num","idx_u","idx_v(ground_truth)","Current Time", "Event Type","True time","Predict time","Conditional_Density","Ranking","Ranked_No1_idx"])     #写入列的名称
...
#                 t_predict = cur_time + (t_next / args.PREDICT_N)
                ...
                if(abs(t_truth_np - t_predict_np)<=10):          
                    with open(args.rst_save_dir + 'testrst.csv','a+') as f:
                        ...
                        data_row = [ith_slot+1,u,v, i[2], i[3], i[4] , t_predict[0]*3600 ,v_condi,rankno,ranking[0].item()]
    ...
    with open(args.rst_save_dir + 'Entertainment_dict_of_set' + '.pkl', 'wb') as f:
        pickle.dump(dict_of_set, f, pickle.HIGHEST_PROTOCOL)
    with open(args.rst_save_dir + 'Entertainment_dict_of_node' + '.pkl', 'wb') as f:
...
#     train_data = train_data[0:600]
    ...
    for epoch in range(args.epochs):
        log_dir = './dyrep_Entertainment_para.pkl'
        model_dir = './Entertainment_model.pkl'

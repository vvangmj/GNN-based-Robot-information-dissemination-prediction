...
def initialize_Graph_matrix(initial_data, node_num):
    G = nx.MultiGraph()
    G.add_nodes_from(range(0,node_num))
    for row in initial_data:
        G.add_edge(row[0],row[1],time = row[2],conn = row[3])

    begin_date = datetime.datetime(2020, 5, 1)
    begin_date = date_timestamp_switch(begin_date)
    Lambda_time = [[[0 for c in range(2)]for r in range(node_num)]for d in range(node_num)]
    for i in range(node_num):
        for j in range(node_num):
            Lambda_time[i][j][1] = begin_date

    node_time = np.empty((node_num), dtype = float)
